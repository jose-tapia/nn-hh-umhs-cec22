...
class Hyperheuristic:
    ...
    def _solve_dynamic(self, save_steps = True):
        """
        Run the hyper-heuristic based on Random Search (RS) to find the best metaheuristic. This method generated 
        'num_replicas' unfolded metaheuristics (uMHs); after generating all uMHs, their fitness values are used to
        obtain statistics and then the performance of the HH. Once the process ends, it returns:
            ...
            - transition_matrix: A matrix that illustrates how have been used the search operators along with the 
              generation of the uMHs.

        :returns: fitness_per_repetition (list), sequence_per_repetition (list), transition_matrix (matrix)
        """
    ...
    def _solve_neural_network(self, save_steps=True):
        """
         ...
         single Neural Network model is trained to generate 100 unfolded metaheuristics (uMHs) for the given 
        ...
        This method requires a special dictionary with the following parameters:
        ...
        # Neural Network model for prediction of search operators
        ...
        for rep in range(self.parameters['num_replicas']):
            ...
            while not self._check_finalisation(step, stag_counter):
                ...
                if current_fitness < best_fitness[-1]:
                    ...
                    # Update stagnation
                    stag_counter += 1
                    if stag_counter % self.parameters.get('tabu_idx', 5) == 0:
    ...
    def _get_neural_network_predictor(self):
        "Train a Neural Network model using 'model_params' for _solve_neural_network"
        ...
        if model_params.get('load_model', False):
            model.load()
            return
        ...
        # Save trained model
        if model_params.get('save_model', False):
            model.save()
        return model
    ...
    def _get_sample_sequences(self, sample_params):
        """
        Retrieve or generate sequences to use them as data train in _solve_neural_network

        :param dict sample_params: Store the following values,
            - retrieve_sequences (bool): True if would retrieve stored sequences, otherwise 
              generate the sequences
            - limit_seqs (int): Limit of sequences that would be generated
            - filters (str): Specify which kind of sequences would be retrieved
            - store_sequences (bool): True if would store the sample of sequences
