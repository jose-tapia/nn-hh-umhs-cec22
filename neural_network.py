...
def retrieve_model_info(params):
    """
    Given the parameters of the Neural Network model, retrieve the respective model name, load and save paths

    :param dict params:
        Parameters of the Neural Network model

    :return: The architecture name, encoder type, and dict with important paths
    """
...
class Encoder():
    def __init__(self, params):
        ...
        if self._architecture_name in ['LSTM_Ragged']:
            # Keep original values but delete the first element (-1)
...
class ModelPredictorKeras():
    ...
    def __create_keras_model(self):
        "Build the model according to the parameters"
    ...
    def fit(self, X, y, epochs=100, sample_weight=None, verbose=False):
        """ 
        Given the training set and the fit parameters, train the Neural Network model
        
        :param list X:
            Input sample sequences
        :param list y:
            Expected output from sample sequences
        :param int epochs:
            Number of epochs to train
        :param list sample_weight:
            Importance of each sample sequence
        :param bool verbose:
            True if verbose is allowed

        :return: None
        
        After train the Neural Network, the private variable _predict saves the predict 
        function from the Neural Network model
        """
        # Pre-process dataset
    ...
    def predict(self, sequence):
        "Given an input sequence, use the model to predict the distribution"
...
def ModelPredictor(params):
    "Wrapping function to select the model predictor according to the parameters"
